...
export default function AccountingClient({
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      {/* ── Tab bar ── */}
      <div className="w-full overflow-x-auto whitespace-nowrap scrollbar-hide border-b border-slate-200/60 bg-slate-50/50 p-1 mb-6 no-print">
        <div className="h-auto p-0 bg-transparent flex w-max min-w-full rounded-none border-none gap-1">
          {tabs.map(t => (
            <button
              key={t.id}
              onClick={() => switchTab(t.id)}
              className={`flex items-center rounded-lg px-3 py-2 transition-all duration-300 gap-1.5 font-bold text-[10px] uppercase tracking-tight group border shadow-sm
                ${tab === t.id
                  ? 'bg-[#001529] text-white border-transparent shadow-md'
                  : 'text-slate-500 border-slate-200 hover:bg-white hover:text-[#001529] bg-slate-100/80'
                }`}
            >
              <t.icon className={`h-3.5 w-3.5 transition-colors ${tab === t.id ? 'text-[#7FD1E3]' : 'group-hover:text-slate-900'}`} />
              {t.label}
            </button>
          ))}
        </div>
