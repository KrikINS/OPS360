...
describe('clockIn', () => {
  it('creates an attendance record for today', async () => {
    ...
    const result = await clockIn() as ClockResult
...
describe('clockOut', () => {
  it('sets clock_out and computes duration_minutes', async () => {
    ...
    const result = await clockOut() as ClockResult
    expect(result.success).toBe(true)
    if (result.success) {
...
describe('getMyAttendance', () => {
  it('returns only the current user records in date range', async () => {
    ...
    const result = await getMyAttendance({ fromDate: today, toDate: today }) as AttendanceListResult
    expect(result.success).toBe(true)
    if (result.success) {
      expect(result.records.length).toBe(1)
...
describe('getActivityLog', () => {
  it('returns activities from multiple sources', async () => {
    ...
    const result = await getActivityLog({ fromDate: today, toDate: today }) as ActivityLogResult
    expect(result.success).toBe(true)
    if (result.success) {
      expect(result.activities.length).toBeGreaterThanOrEqual(1)
      const hrActivity = result.activities.find(
        (a) => a.module === 'HR' && a.actionType === 'clock_in'
      )
  ...
  it('manager can filter by specific staff member', async () => {
    ...
    const result = await getActivityLog({ userId: STAFF_ID, fromDate: today, toDate: today }) as ActivityLogResult
    expect(result.success).toBe(true)
    if (result.success) {
  ...
  it('returns empty array when no activity in date range', async () => {
    ...
    const result = await getActivityLog({ fromDate: '2020-01-01', toDate: '2020-01-02' }) as ActivityLogResult
    expect(result.success).toBe(true)
    if (result.success) {
