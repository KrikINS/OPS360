...
import {
  ...
  credit_payments, customers, sales_invoices
} from '@/db/schema'
...
export async function postSalesJournal(input: {
  ...
  // 1) Find the actual cogs if not supplieded, calculate from inventory
...
// ── postServiceJournal ──────────────────────────────
export async function postServiceJournal(input: {
  jobId: string
  jobNumber: string
  branchId: string
  actualCost: number
  ...
  if (input.actualCost <= 0) return { success: false as const, error: 'Actual cost must be > 0' }

  const cashAccountCode = input.paymentMethod === 'cash'
    ? await getCashAccountCode(input.branchId)
    : '1020'

  const lines: Array<{ accountCode: string; debit?: number; credit?: number; description: string }> = [
    { accountCode: cashAccountCode, debit: input.actualCost, description: `Service payment received — ${input.jobNumber}` },
    { accountCode: '4010',          credit: input.actualCost, description: `Service revenue — ${input.jobNumber}` },
  ]

  const entry = await createJournalEntry({
    description:     `Service Job: ${input.jobNumber} — ₹${input.actualCost.toLocaleString('en-IN')}`,
    referenceSource: 'SERVICE',
  ...
  return { success: true as const, journalEntryId: entry.id }
}
...
export async function postTransferJournal(input: {
  ...
  // SOURCE branch journal — asset leaves
  await createJournalEntry({
    description:
      `Stock transfer out: ${input.transferNumber} ` +
      `(${input.itemCount} unit(s)) → receiving branch`,
    referenceSource: 'TRANSFER',
    ...
    lines: [
      {
        accountCode: '1040',
        credit:      Math.round(input.totalLandedCost * 100) / 100,
        description: `Inventory transferred out — ` +
          `${input.transferNumber}`,
      },
      {
        accountCode: '1060',
        debit:       Math.round(input.totalLandedCost * 100) / 100,
  ...
  // DESTINATION branch journal — asset arrives
  await createJournalEntry({
    description:
      `Stock transfer in: ${input.transferNumber} ` +
      `(${input.itemCount} unit(s)) ← sending branch`,
    referenceSource: 'TRANSFER',
    referenceId:     input.transferId,
...
export async function postCreditSaleJournal(input: {
  invoiceId: string
  branchId: string
  createdBy: string
  saleTotal: number
  ...
  if (cogsAmount === 0 && input.invoiceId) {
    try {
      const soldUnits = await db.select({ landedCost: inventory.landed_cost })
        .from(inventory).where(eq(inventory.invoice_id, input.invoiceId))
      cogsAmount = soldUnits.reduce((sum, unit) => sum + Number(unit.landedCost ?? 0), 0)
    } catch { cogsAmount = 0 }
  }

  const lines: Array<{ accountCode: string; debit?: number; credit?: number; description?: string }> = [
    { accountCode: '1030', debit: input.saleTotal, description: 'Credit sale — AR' },
    { accountCode: '4000', credit: input.subtotal,  description: 'Sales revenue ex-tax' },
  ]
  if (input.cgst > 0) lines.push({ accountCode: '2020', credit: input.cgst, description: 'CGST collected' })
  if (input.sgst > 0) lines.push({ accountCode: '2030', credit: input.sgst, description: 'SGST collected' })
  ...
  await createJournalEntry({
    description:     'Auto: Credit sale invoice',
    referenceSource: 'SALES',
    referenceId:     input.invoiceId,
    branchId:        input.branchId,
    autoGenerated:   true,
...
export async function finalizeCreditCheckout(input: {
  invoiceId: string
  branchId: string
  createdBy: string
  saleTotal: number
  ...
  await postCreditSaleJournal(input)

  if (input.customerId) {
    await db.execute(sql`
      UPDATE customers
      SET credit_balance = credit_balance + ${input.saleTotal}
      WHERE id = ${input.customerId}::uuid
    `)
  }

  const dueDate = new Date()
  const terms = Number(input.creditTerms.replace(/\D/g, '') || 30)
  ...
  await db.update(sales_invoices)
    ...
    .where(eq(sales_invoices.id, input.invoiceId))
}
...
export async function recordCreditPayment(input: {
  ...
  try {
    ...
    const [invoice] = await db.select({
      ...
      amount_paid:  sales_invoices.amount_paid,
      payment_status: sales_invoices.payment_status,
      customer_id: sales_invoices.customer_id,
    }).from(sales_invoices).where(eq(sales_invoices.id, input.invoiceId)).limit(1)

    if (!invoice) return { success: false as const, error: 'Invoice not found' }
    if (invoice.payment_status === 'paid') return { success: false as const, error: 'Invoice already fully paid' }
    ...
    // Post journal: DR Cash, CR 1030 AR
    const cashCode = input.paymentMode === 'cash'
      ? await getCashAccountCode(input.branchId)
      : '1020'

    const entry = await createJournalEntry({
      description:     `Credit payment received — Invoice ${input.invoiceId.slice(0, 8)}`,
      referenceSource: 'CREDIT_PAYMENT',
      referenceId:     input.invoiceId,
      branchId:        input.branchId,
      autoGenerated:   true,
      createdBy:       session.user.id,
      lines: [
        { accountCode: cashCode, debit: paymentAmount,  description: 'Credit payment received' },
        { accountCode: '1030',   credit: paymentAmount, description: 'AR settled' },
      ],
    })

    // Update invoice
    await db.update(sales_invoices)
      .set({ amount_paid: String(newAmountPaid), payment_status: newStatus })
      .where(eq(sales_invoices.id, input.invoiceId))

    // Update customer credit_balance
    await db.execute(sql`
      UPDATE customers
      SET credit_balance = GREATEST(0, credit_balance - ${paymentAmount})
      WHERE id = ${input.customerId}::uuid
    `)

    // Record payment
    await db.insert(credit_payments).values({
    ...
    return { success: true as const, paymentAmount, newStatus, journalEntryId: entry.id }
  } catch (error) {
    return { success: false as const, error: (error as Error).message }
  }
}

export async function getAROutstanding(input?: { customerId?: string; branchId?: string }) {
  const session = await getServerSession(authOptions)
  if (!session?.user) return { success: false as const, error: 'Unauthorized' }
  ...
  try {
    const rows = unpackRows<{
      ...
      SELECT
        si.id, si.invoice_number, si.created_at, si.due_date,
        si.total_amount, si.amount_paid, si.payment_status,
        si.customer_id,
        c.full_name AS customer_name,
        c.phone_number AS customer_phone,
        CASE
          WHEN si.due_date IS NULL THEN 0
          WHEN si.due_date < CURRENT_DATE THEN (CURRENT_DATE - si.due_date)
          ELSE 0
        END AS days_overdue
      FROM sales_invoices si
      JOIN customers c ON c.id = si.customer_id
      WHERE si.payment_mode = 'credit'
        AND si.payment_status IN ('unpaid', 'partial')
        ${input?.customerId ? sql`AND si.customer_id = ${input.customerId}::uuid` : sql``}
        ${input?.branchId  ? sql`AND si.branch_id   = ${input.branchId}::uuid`   : sql``}
      ORDER BY si.due_date ASC NULLS LAST, si.created_at ASC
    `))

    const totalOutstanding = rows.reduce((s, r) => s + Number(r.total_amount) - Number(r.amount_paid), 0)
    const overdueCount     = rows.filter(r => Number(r.days_overdue) > 0).length

    return { success: true as const, invoices: rows, totalOutstanding, overdueCount }
  } catch (error) {
    return { success: false as const, error: (error as Error).message }
  }
}
...
export async function updateCustomerCredit(input: {
  ...
  try {
    await db.update(customers)
      .set({
        is_credit_eligible:   input.isCreditEligible,
        credit_limit:         String(input.creditLimit),
        credit_payment_terms: input.creditPaymentTerms,
      })
